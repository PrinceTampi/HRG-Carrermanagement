...
(function recruitmentPublicScripts() {
  ...
  function setupPsychotestDeviceCheck() {
    const deviceCheck = document.querySelector("[data-psychotest-device-check]");
    if (!deviceCheck) {
      return;
    }
    ...
    const browserStatus = document.querySelector("[data-device-browser-status]");
    ...
    const updateStatus = function updateStatus(statusNode, state, title, message) {
      statusNode.classList.remove("is-success", "is-warning", "is-error");
      statusNode.classList.add(
        state === "success" ? "is-success" : state === "error" ? "is-error" : "is-warning",
      );
    ...
    const handleRequestError = function handleRequestError(error) {
      ...
      if ("NotAllowedError" === name) {
        ...
        message = "Akses kamera dan mikrofon dibatalkan. Silakan izinkan kembali.";
        ...
        message = "Kamera sedang digunakan oleh perangkat lain. Tutup aplikasi lain dan coba lagi.";
      } else if ("SecurityError" === name) {
        title = "Akses dibatasi";
        message = "Halaman ini harus diakses melalui localhost atau HTTPS agar kamera dapat dipakai.";
      }

      updateStatus(cameraStatus, "error", title, message);
      updateStatus(microphoneStatus, "warning", "Mikrofon belum aktif", "Izinkan akses mikrofon saat kamera dibuka.");
      updateStatus(audioStatus, "warning", "Audio belum diperiksa", "Audio akan diperiksa setelah izin diberikan.");
    ...
    document.addEventListener("visibilitychange", function onPageVisibilityChanged() {
      if (document.hidden) {
        stopStream();
      }
    });
