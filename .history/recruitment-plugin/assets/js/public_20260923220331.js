...
(function recruitmentPublicScripts() {
	...
	function setupScreenExplorer() {
		...
		const groupButtons = explorer.querySelectorAll('[data-screen-explorer-group-toggle]');
		groupButtons.forEach(function groupButton(groupButtonElement) {
			const parentGroup = groupButtonElement.closest('.daw-screen-explorer__group');
			const body = parentGroup ? parentGroup.querySelector('.daw-screen-explorer__group-body') : null;
			if (!parentGroup || !body) {
				return;
			}
